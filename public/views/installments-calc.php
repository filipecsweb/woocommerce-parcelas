...
$installments_overwrite = wcParcelas()->metaBox->get_fswp_post_meta_data( wcParcelas()->metaBox->custom_installment_qty_key );
$installments           = ! isset( $installments_overwrite ) ? wcParcelas()->settings->getOption( 'installment_qty' ) : $installments_overwrite;
...
if ( $price <= $min_value ) {
	...
	if ( $installments_price < $min_value ) {
		...
		if ( $installments_price >= $min_value ) {
			...
			$installments_html .= "<p class='price fswp_calc'>" . sprintf( __( '<span class="fswp_installment_prefix">%s %sx de</span> ', 'wc-parcelas' ),
					$prefix, $installments ) . $formatted_installments_price . " <span class='fswp_installment_suffix'>" . $suffix . "</span></p>";
		...
		$installments_html = "<div class='fswp_installments_price $class'>";
		$installments_html .= "<p class='price fswp_calc'>" . sprintf( __( '<span class="fswp_installment_prefix">%s %sx de </span>', 'wc-parcelas' ), $prefix,
				$installments ) . $formatted_installments_price . " <span class='fswp_installment_suffix'>" . $suffix . "</span></p>";
		$installments_html .= "</div>";
	}
}
